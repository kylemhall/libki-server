...
subtest 'Successful release with no gratis discount' => sub {
    ...
    is( scalar @$stats, 0, 'No gratis statistic created' );
};
...
subtest 'Successful release with gratis discount - pages method (partial)' => sub {
    ...
    is( scalar @$stats, 1, 'GRATIS_DISCOUNT statistic was created' );
    is( $stats->[0]->{action}, 'GRATIS_DISCOUNT', 'Statistic action is correct' );
};

subtest 'Successful release with gratis discount - balance method' => sub {
    my ( $user, $debit_called, $debit_amount, $gratis_bal )
        = build_mock_user( funds => 10.00, gratis_print_balance => 30 );  # 30 cents gratis
    my $print_file = build_mock_print_file( pages => 5 );
    my ( $print_job, $pj_status ) = build_mock_print_job(
        copies     => 1,
        print_file => $print_file,
    ...
    is( $result->{success}, 1, 'Should succeed' );
    # 5 pages * 0.10 = 0.50 cost, gratis balance 30 > 0.50, so cost = 0, discount = 0.50
    is( $$debit_amount, 0, 'Debited zero after full gratis balance discount' );
    is( scalar @$stats, 1, 'GRATIS_DISCOUNT statistic was created' );
};
